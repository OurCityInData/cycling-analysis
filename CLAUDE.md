...
- `tests/test_area_classification.py` — the original 11 real OSM ways from the Barcelona classifier's ground truth (tags/urls copied verbatim), plus 5 more Barcelona ways added later, run through `classify_area_road()`; 12/16 correct, 4 xfail: the original 2 (Rijwielpad Noordvoort, Shared greenway near Rimini — both tag-only Greenway gaps, see Known gaps below) plus 2 new ones — "Avinguda Diagonal" (no `oneway` tag at all, so the two-way default fires instead of one-way) and "Carrer de Sants" (tagged `cycleway:both=share_busway`, which `classify_area_road()` always reads as `Bus-bike lane` regardless of maxspeed, even though this way is meant to be a plain 30 km/h shared street — the tag can't currently distinguish the two real-world cases)
