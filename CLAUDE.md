...
- `tests/test_area_classification.py` — the original 11 real OSM ways from the Barcelona classifier's ground truth (tags/urls copied verbatim), plus 5 more Barcelona ways and 2 real Amsterdam ways (Han Lammersbrug, Polonceau-kade — both `highway=footway` + `bicycle=yes`) added later, run through `classify_area_road()`; 15/18 correct, 3 xfail: the original 2 (Rijwielpad Noordvoort, Shared greenway near Rimini — both tag-only Greenway gaps, see Known gaps below) plus "Avinguda Diagonal" (no `oneway` tag at all, so the two-way default fires instead of one-way). "Carrer de Sants" was previously xfail here too (mislabeled ground truth expecting "Shared street at 30 km/h") but is now a pass at "Bus-bike lane" — `cycleway:both=share_busway` is a genuine bus-bike shared lane, and `classify_area_road()`'s step-6 match was correct all along
