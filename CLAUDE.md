...
- `cycling_analysis/population.py` — `fetch_population_raster()`, `compute_population_stats()`: WorldPop-based `Population`/`Population density (per km²)` columns, all 5 countries (see Population columns above)
...
- `tests/test_population.py` — unit tests for `compute_population_stats()` against an in-memory synthetic raster (no network; `fetch_population_raster()` hits WorldPop directly and was validated manually, see Population columns above)
...
Adding the population columns bumped the checkpoint schema (see checkpoint-versioning behavior above) — the existing `spain_cataluna_checkpoint.csv`/`spain_la_rioja_checkpoint.csv` (and their matching `*_cycling_by_municipality.csv`) predate `Population`/`Population density (per km²)` and must be deleted or moved aside before the next run of either; a fresh run will re-fetch/re-zonal-sum population for every municipality (cheap — the per-country raster download + bulk zonal-stats call, not per-municipality).
